...
const AchievementBar = ({ rate, collected, target }: AchievementBarProps) => {
  ...
  return (
    <motion.div
      ...
      className="mb-4 rounded-lg border border-border bg-card p-5 shadow-card"
    >
      <div className="mb-3.5 flex flex-wrap items-start justify-between gap-2">
        <div>
          <div className="text-[9px] font-bold tracking-[3px] uppercase text-muted-foreground">
        ...
        <div className="font-display text-[44px] leading-none text-foreground">
          {rate}<span className="text-[20px] text-muted-foreground">%</span>
        </div>
      </div>
      <div className="mb-2 h-3.5 overflow-hidden rounded-full bg-secondary">
        <motion.div
